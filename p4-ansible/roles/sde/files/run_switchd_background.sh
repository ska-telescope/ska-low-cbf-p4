...
function print_help() {
  ...
  echo "  --background"
  echo "    Run switchd in background"
  exit 0
}
...
[ -z "${SDE_VERSION}" ] && echo "Environment variable SDE_VERSION not set" && exit 1
[ -z "${SDE_PATH}" ]    && echo "Environment variable SDE_PATH not set" && exit 1

export SDE=${SDE_PATH}${SDE_VERSION}
export SDE_INSTALL=${SDE_PATH}${SDE_VERSION}/${INSTALL_SUFFIX}
echo "Using SDE ${SDE}"
echo "Using SDE_INSTALL ${SDE_INSTALL}"

opts=$(getopt -o c:Cp:ghr:s --long background,no-status-srv,server-listen-local-only,gdb-server,skip-p4,skip-port-add,kernel-pkt,skip-hld:,status-port:,init-mode:,arch: -- "$@")
if [ $? != 0 ]; then
  exit 1
fi
eval set -- "$opts"
...
if [[ $REDIRECTLOG != "" ]]; then
  sudo env "SDE=$SDE" "SDE_INSTALL=$SDE_INSTALL" $ASAN_ON_ERROR "PATH=$PATH" \
	...
	$SKIP_HLD_STR $SKIP_P4_STR $SKIP_PORT_ADD_STR $STS_PORT_STR $KERNEL_PKT_STR $SHELL_NO_WAIT $BACKGROUND $@ &> $REDIRECTLOG &
else
  sudo env "SDE=$SDE" "SDE_INSTALL=$SDE_INSTALL" $ASAN_ON_ERROR "PATH=$PATH" \
      "LD_LIBRARY_PATH=$LD_LIBRARY_PATH" $DBG bf_switchd \
    "$SERVER_LISTEN_LOCAL_ONLY" \
	--install-dir $SDE_INSTALL --conf-file $TARGET_CONFIG_FILE "--init-mode=$INIT_MODE" \
	$SKIP_HLD_STR $SKIP_P4_STR $SKIP_PORT_ADD_STR $STS_PORT_STR $KERNEL_PKT_STR $SHELL_NO_WAIT $BACKGROUND $@
fi
